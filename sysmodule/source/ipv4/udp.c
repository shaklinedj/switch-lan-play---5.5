/* Adapted from src/ipv4/udp.c — only the include path changed. */
#include "ipv4.h"
...
)
{
    ...
    WRITE_NET16(buffer, UDP_OFF_SRCPORT,  srcport);
    WRITE_NET16(buffer, UDP_OFF_DSTPORT,  dstport);
    WRITE_NET16(buffer, UDP_OFF_LENGTH,   udp_length);
    WRITE_NET16(buffer, UDP_OFF_CHECKSUM, 0);
    ...
    WRITE_NET16(buffer, UDP_OFF_CHECKSUM, checksum);

    return send_ipv4_ex(self, src, dst, IPV4_PROTOCOL_UDP, &part);
}
