/* Adapted from src/ipv4/ipv4.c for the Switch sysmodule.
 * Change: gateway_on_packet() is NOT available on Switch.
 *         That branch (routing to internet via SOCKS5) is replaced
 *         with a no-op; all other logic is identical to the original. */
#include "ipv4.h"

int send_ipv4(
    struct packet_ctx *arg,
    const void *dst,
    uint8_t protocol,
    const struct payload *payload
)
{
    return send_ipv4_ex(arg, arg->ip, dst, protocol, payload);
}

int send_ipv4_ex(
    struct packet_ctx *arg,
    const void *src,
    const void *dst,
    uint8_t protocol,
    const struct payload *payload
)
{
    struct payload part;
    uint8_t dst_mac[6];
    uint8_t buffer[IPV4_HEADER_LEN];

    WRITE_NET8(buffer,  IPV4_OFF_VER_LEN,          0x45);
    WRITE_NET8(buffer,  IPV4_OFF_DSCP_ECN,         0x00);
    WRITE_NET16(buffer, IPV4_OFF_TOTAL_LEN,        IPV4_HEADER_LEN + payload_total_len(payload));
    WRITE_NET16(buffer, IPV4_OFF_ID,               arg->identification++);
    WRITE_NET16(buffer, IPV4_OFF_FLAGS_FRAG_OFFSET, 0);
    WRITE_NET8(buffer,  IPV4_OFF_TTL,              128);
    WRITE_NET8(buffer,  IPV4_OFF_PROTOCOL,         protocol);
    WRITE_NET16(buffer, IPV4_OFF_CHECKSUM,         0x0000);
    CPY_IPV4(buffer + IPV4_OFF_SRC, src);
    CPY_IPV4(buffer + IPV4_OFF_DST, dst);

    uint16_t checksum = calc_checksum(buffer, IPV4_HEADER_LEN);
    WRITE_NET16(buffer, IPV4_OFF_CHECKSUM, checksum);

    part.ptr  = buffer;
    part.len  = IPV4_HEADER_LEN;
    part.next = payload;

    if (!arp_get_mac_by_ip(arg, dst_mac, dst)) return 0;

    return send_ether(arg, dst_mac, ETHER_TYPE_IPV4, &part);
}

void parse_ipv4(const struct ether_frame *ether, struct ipv4 *ipv4)
{
    const u_char *packet = ether->payload;
    uint8_t t;
    uint16_t tt;

    ipv4->ether      = ether;
    t                = READ_NET8(packet, IPV4_OFF_VER_LEN);
    ipv4->version    = t >> 4;
    ipv4->header_len = (t & 0xF) * 4;
    t                = READ_NET8(packet, IPV4_OFF_DSCP_ECN);
    ipv4->dscp       = t >> 2;
    ipv4->ecn        = t & 3;
    ipv4->total_len  = READ_NET16(packet, IPV4_OFF_TOTAL_LEN);
    ipv4->identification = READ_NET16(packet, IPV4_OFF_ID);
    tt               = READ_NET16(packet, IPV4_OFF_FLAGS_FRAG_OFFSET);
    ipv4->flags      = (uint8_t)(tt >> 13);
    ipv4->fragment_offset = tt & 0x1fff;
    ipv4->ttl        = READ_NET8(packet,  IPV4_OFF_TTL);
    ipv4->protocol   = READ_NET8(packet,  IPV4_OFF_PROTOCOL);
    ipv4->checksum   = READ_NET16(packet, IPV4_OFF_CHECKSUM);
    CPY_IPV4(ipv4->src, packet + IPV4_OFF_SRC);
    CPY_IPV4(ipv4->dst, packet + IPV4_OFF_DST);
    ipv4->payload    = packet + ipv4->header_len;
}

int process_ipv4(struct packet_ctx *arg, const struct ether_frame *ether)
{
    struct ipv4 ipv4;
    parse_ipv4(ether, &ipv4);
    arp_set(arg, ether->src, ipv4.src);

    if (CMP_IPV4(ipv4.dst, arg->ip)) {
        /* Packet addressed to us (gateway IP) */
        switch (ipv4.protocol) {
            case IPV4_PROTOCOL_ICMP:
                return process_icmp(arg, &ipv4);
        }
    } else if (IS_SUBNET(ipv4.dst, arg->subnet_net, arg->subnet_mask)) {
        if (IS_BROADCAST(ipv4.dst, arg->subnet_net, arg->subnet_mask)) {
            /* Subnet broadcast → send to relay AND reflect back */
            lan_client_send_ipv4(arg->arg, (void *)ipv4.dst,
                                 ether->payload, ipv4.total_len);
            struct payload part;
            part.ptr  = ether->payload;
            part.len  = ipv4.total_len;
            part.next = NULL;
            return send_ether(arg, ether->src, ETHER_TYPE_IPV4, &part);
        } else if (arp_has_ip(arg, ipv4.dst)) {
            uint8_t dst_mac[6];
            struct payload part;
            arp_get_mac_by_ip(arg, dst_mac, ipv4.dst);
            part.ptr  = ether->payload;
            part.len  = ipv4.total_len;
            part.next = NULL;
            return send_ether(arg, dst_mac, ETHER_TYPE_IPV4, &part);
        } else {
            return lan_client_send_ipv4(arg->arg, (void *)ipv4.dst,
                                        ether->payload, ipv4.total_len);
        }
    }
    /* Packets outside our subnet: no gateway on Switch — drop. */
    return 0;
}

uint16_t calc_checksum(const u_char *buffer, int len)
{
    uint32_t sum = 0;
    const uint16_t *buf = (const uint16_t *)buffer;
    while (len > 1) {
        sum += ntohs(*buf++);
        len -= 2;
    }
    if (len) sum += *(const uint8_t *)buf;
    while (sum > 0xffff) sum -= 0xffff;
    return (uint16_t)(~sum);
}

uint16_t calc_payload_checksum(const struct payload *payload)
{
    uint32_t sum = 0;
    int offset = 0;
    const struct payload *part = payload;

    while (part) {
        const uint16_t *buf = (const uint16_t *)part->ptr + offset;
        int len = part->len - offset;
        while (len > 1) {
            sum += ntohs(*buf++);
            len -= 2;
        }
        part = part->next;
        if (len) {
            if (part) {
                sum += ((uint16_t)READ_NET8(buf, 0) << 8)
                       | (uint16_t)READ_NET8(part->ptr, 0);
            } else {
                sum += (uint16_t)READ_NET8(buf, 0) << 8;
            }
            offset = 1;
        } else {
            offset = 0;
        }
    }

    while (sum > 0xffff) sum -= 0xffff;
    return (uint16_t)(~sum);
}
