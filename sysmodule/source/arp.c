/* Adapted from src/arp.c for the Switch sysmodule.
...
)
{
    ...
    return send_ether(self, dst_mac, ETHER_TYPE_ARP, &part);
}
...
{
    return send_arp_ex(self, target_mac, opcode,
                       sender_mac, sender_ip, target_mac, target_ip);
}

int send_arp_request(struct packet_ctx *self, const void *target_ip)
{
    return send_arp_ex(self, BROADCASE_MAC, ARP_OPCODE_REQUEST,
                       self->mac, self->ip, NONE_MAC, target_ip);
}

int arp_request(struct packet_ctx *self, const struct arp *arp)
{
    if (IS_SUBNET(arp->target_ip, self->subnet_net, self->subnet_mask)) {
        if (CMP_IPV4(arp->target_ip, arp->sender_ip))  return 0;
        if (CMP_IPV4(arp->sender_ip, NONE_IP))         return 0;
        if (arp_has_ip(self, arp->target_ip))           return 0;
        return send_arp(self, ARP_OPCODE_REPLY,
                        self->mac,     arp->target_ip,
...
int process_arp(struct packet_ctx *self, const struct ether_frame *ether)
{
    ...
    if (
        ...
        LLOG(LLOG_WARNING, "Unknown ARP hardware/protocol "
             "(hw=%d proto=%04x)", arp.hardware_type, arp.protocol_type);
        return -1;
    }
    ...
    switch (arp.opcode) {
        case ARP_OPCODE_REQUEST: return arp_request(self, &arp);
        case ARP_OPCODE_REPLY:   return arp_reply(self, &arp);
    }
    return -1;
}
...
bool arp_has_ip(struct packet_ctx *self, const void *ip)
{
    ...
    for (int i = 0; i < ARP_CACHE_LEN; i++) {
        struct arp_item *item = &self->arp_list[i];
...
bool arp_get_mac_by_ip(struct packet_ctx *self, void *mac, const void *ip)
{
    ...
    if (ret) LLOG(LLOG_DEBUG, "send_arp_request returned %d", ret);
    return false;
}
...
bool arp_set(struct packet_ctx *self, const void *mac, const void *ip)
{
    ...
    for (int i = 0; i < ARP_CACHE_LEN; i++) {
        struct arp_item *item = &self->arp_list[i];
        if (CMP_IPV4(item->ip, NONE_IP) || CMP_IPV4(item->ip, ip)
                || item->expire_at < now) {
            CPY_IPV4(item->ip, ip);
    ...
    LLOG(LLOG_WARNING, "arp_set: cache full");
    return false;
}
