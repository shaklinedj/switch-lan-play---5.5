...
int UdpLanSocketBase::sendto(const void *buf, size_t len, struct sockaddr_in *addr) {
    int rc = ::sendto(this->fd, buf, len, 0, (struct sockaddr *)addr, sizeof(*addr));
    /* Mirror every LDN UDP packet to the sysmodule bridge on 127.0.0.1:11453
     * so the bridge can forward them to the relay server. */
    struct sockaddr_in bridge;
    bridge.sin_family      = AF_INET;
    bridge.sin_addr.s_addr = htonl(0x7f000001u); /* 127.0.0.1 */
    bridge.sin_port        = htons(11453);
    ::sendto(this->fd, buf, len, 0, (struct sockaddr *)&bridge, sizeof(bridge));
    return rc;
}
