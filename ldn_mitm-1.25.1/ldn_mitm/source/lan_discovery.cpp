...
namespace ams::mitm::ldn {
            ...
            switch (type) {
                case LANPacketType::Scan: {
                    if (this->discovery->getState() == CommState::AccessPointCreated) {
                        ...
                        if (data && size >= sizeof(ScanFilter)) {
                            filter = static_cast<const ScanFilter *>(data);
                        }

                        /* Compatibility note:
                         * Older clients send an empty Scan packet. In that
                         * case filter remains nullptr and we reply exactly as
                         * before. New clients include the game's ScanFilter so
                         * hosts from other games/scenes do not answer, reducing
                         * noise on public legacy relays such as tekn0.net
                         * without requiring any server changes. */
                        if (scanFilterMatchesNetwork(filter, &this->discovery->networkInfo)) {
                            reply(LANPacketType::ScanResp, &this->discovery->networkInfo, sizeof(NetworkInfo));
                        } else {
                            LogFormat("Scan filtered: intent/network mismatch");
                        }
                    }
                    break;
                }
    ...
    Result LANDiscovery::scan(NetworkInfo *pOutNetwork, u16 *count, ScanFilter filter) {
        ...
        int len = this->udp->sendBroadcast(LANPacketType::Scan, &filter, sizeof(filter));
        if (len < 0) {
        ...
        for (auto& item : this->udp->scanResults) {
            ...
            bool copy = scanFilterMatchesNetwork(&filter, &info);
    ...
    Result LANDiscovery::connect(const NetworkInfo *networkInfo, UserConfig *userConfig, u16 localCommunicationVersion) {
        ...
        /* If the host IP is in the 10.13.0.0/16 virtual relay subnet,
         * route the TCP connection through the sysmodule bridge proxy
         * on 127.0.0.1:11453 which knows how to reach that player. */
        if ((hostIp & 0xFFFF0000u) == 0x0A0D0000u) { /* 10.13.x.x */
            addr.sin_addr.s_addr = htonl(0x7f000001u); /* 127.0.0.1 */
            addr.sin_port        = htons(11453);
            LogFormat("connect via_relay: hostIp %x -> 127.0.0.1:11453", hostIp);
        } else {
